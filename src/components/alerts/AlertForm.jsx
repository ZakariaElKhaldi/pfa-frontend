  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <div className="flex items-center gap-2">
        <Label className="text-xs text-subtle whitespace-nowrap">Type</Label>
        <Select
          value={filters.type ?? 'all'}
          onValueChange={(v) => onChange({ ...filters, type: v === 'all' ? undefined : v })}
        >
          <SelectTrigger className="h-8 w-40 text-xs bg-surface border-container">
            <SelectValue />
          </SelectTrigger>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <SelectTrigger className="h-8 w-36 text-xs bg-surface border-container">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-container border-container">
