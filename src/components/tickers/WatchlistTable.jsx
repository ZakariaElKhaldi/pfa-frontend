...
function WatchlistRow({ item, onRemove }) {
  ...
  return (
    <TableRow
      ...
      className="cursor-pointer hover:bg-container transition-colors group"
    >
      {/* Symbol */}
      <TableCell>
      ...
      <TableCell className="font-data text-sm text-right">
        {quote?.price != null ? (
          <span className="text-primary-text">${formatPrice(quote.price)}</span>
        ) : (
      ...
      <TableCell className="text-right">
        {change != null ? (
          ...
          <span className="text-muted font-data text-sm">—</span>
        )}
      </TableCell>
      ...
      <TableCell>
        <button
          ...
          className="text-muted hover:text-signal-sell transition-colors opacity-0 group-hover:opacity-100"
        >
          <StarOff size={14} />
        </button>
      </TableCell>
  ...
  return Array.from({ length: 5 }).map((_, i) => (
    <TableRow key={i}>
      {Array.from({ length: 6 }).map((_, j) => (
        <TableCell key={j}><Skeleton className="h-4 w-full bg-container" /></TableCell>
      ))}
    </TableRow>
  ))
}
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-container hover:bg-transparent">
            ...
            <TableHead className="text-[10px] uppercase tracking-wider text-muted font-semibold">Signal</TableHead>
            <TableHead className="text-[10px] uppercase tracking-wider text-muted font-semibold text-right">Price</TableHead>
            <TableHead className="text-[10px] uppercase tracking-wider text-muted font-semibold text-right">Change</TableHead>
            <TableHead className="text-[10px] uppercase tracking-wider text-muted font-semibold">Added</TableHead>
            <TableHead />
          </TableRow>
        </TableHeader>
        <TableBody>
